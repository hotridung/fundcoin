...
#if defined(NDEBUG)
# error "Fundcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb036397ffdea45ef4d1f6ebfaa9cc2546bc542cd526ee0f0c0aea34cbffe63ed");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fundcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Fundcoin Signed Message:\n";
...
{
    // Fundcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Fundcoin: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Fundcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Fundcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Fundcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fundcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe85dea126813eb1be9e67ebcd38ec50fdfbef9bb96b81fbc1656e6cab1988b81");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is the myself coin for D. community";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1560573779;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085693546;

        if (fTestNet)
        {
            block.nTime    = 1560573727;
            block.nNonce   = 385555271;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd8e586219a14d3e571df14cc323cc38464c1a4e7717f8323a8dd736b18d8d00c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Fundcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FundcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FundcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FundcoinMiner(CWallet *pwallet)
{
    printf("FundcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fundcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FundcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FundcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FundcoinMiner, pwallet));
}
